...
直接使用 psycopg2 建立資料庫表格和插入資料，避免 DAO 層的複雜性。
"""
...
def create_tables():
    ...
    try:
        ...
        """
        cursor.execute(passenger_flow_sql)
...
def insert_sample_stations():
    ...
    try:
        for station_data in stations_data:
            cursor.execute(insert_sql, station_data)
...
def insert_sample_passenger_flow():
    ...
    try:
        ...
        stations_result = cursor.fetchall()
        ...
        while current_date <= end_date:
            for station_code in station_codes:
                ...
                cursor.execute(insert_sql, (station_code, current_date, base_in, base_out))
                total_records += 1
...
def verify_setup():
    ...
    try:
        ...
        stations_result = cursor.fetchone()
        ...
        # 檢查日期範圍
        cursor.execute("SELECT MIN(date) as min_date, MAX(date) as max_date FROM passenger_flow;")
