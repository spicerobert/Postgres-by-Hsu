...
from taiwan_railway_gui.dao.station_dao import StationDAO, create_station_dao
from taiwan_railway_gui.dao.passenger_flow_dao import PassengerFlowDAO, create_passenger_flow_dao
...
class DatabaseIntegrationTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 建立真實的 SQLite 連線
        import sqlite3
        self.sqlite_conn = sqlite3.connect(self.test_db_path)
        self.sqlite_conn.row_factory = sqlite3.Row  # 模擬 RealDictCursor

        # 建立模擬的資料庫管理器
        self.db_manager = Mock()
        self.db_manager.execute_query = self._mock_execute_query
        self.db_manager.get_connection = Mock(return_value=self.sqlite_conn)
        self.db_manager.close_connection = Mock()

        # 建立DAO實例（這些會使用模擬的資料庫管理器）
        with patch('taiwan_railway_gui.dao.database_manager.get_database_manager', return_value=self.db_manager):
            self.station_dao = create_station_dao()
            self.passenger_flow_dao = create_passenger_flow_dao()

    def _mock_execute_query(self, query, params=None, fetch_one=False):
        """模擬資料庫查詢執行"""
        try:
...
class TestStationDAOIntegration(DatabaseIntegrationTestCase):
    ...
    def test_search_stations_by_name(self):
        ...
        # 測試模糊搜尋
        stations = self.station_dao.search_stations('台')
        self.assertGreaterEqual(len(stations), 2)  # 台北、台中

        # 測試找不到的情況
        stations = self.station_dao.search_stations('不存在的車站')
        self.assertEqual(len(stations), 0)

    def test_get_station_by_code(self):
        """測試根據代碼取得車站"""
        # 測試存在的車站
    ...
    def test_get_stations_by_codes(self):
        """測試根據代碼列表取得多個車站"""
        station_codes = ['1000', '1001', '1008']
        ...
        self.assertEqual(stations[2].station_code, '1008')

        # 測試空列表
        stations = self.station_dao.get_stations_by_codes([])
        self.assertEqual(len(stations), 0)
...
class TestPassengerFlowDAOIntegration(DatabaseIntegrationTestCase):
    ...
    def test_get_passenger_flow_by_station_and_date_range(self):
        ...
        flows = self.passenger_flow_dao.get_passenger_flow(
            '1000', start_date.date(), end_date.date()
        )
        ...
        self.assertEqual(first_flow.station_code, '1000')
        self.assertIsInstance(first_flow.in_passengers, int)
        self.assertIsInstance(first_flow.out_passengers, int)
    ...
    def test_get_station_statistics(self):
        ...
        self.assertIsInstance(statistics.total_in, int)
        self.assertIsInstance(statistics.total_out, int)
        self.assertIsInstance(statistics.total_passengers, int)
        self.assertIsInstance(statistics.average_daily, float)
    ...
    def test_get_top_stations(self):
        ...
        top_stations = self.passenger_flow_dao.get_top_stations(
            start_date.date(), end_date.date(), limit=3
        )
        ...
        if top_stations:
            ...
            for i in range(len(top_stations) - 1):
                current_total = top_stations[i].total_passengers
                next_total = top_stations[i + 1].total_passengers
    ...
    def test_get_multiple_station_statistics(self):
        ...
        statistics_list = self.passenger_flow_dao.get_multiple_station_statistics(
            station_codes, start_date.date(), end_date.date()
        )

        self.assertIsInstance(statistics_list, list)
        self.assertEqual(len(statistics_list), 3)

        # 檢查每個車站的資料
        for statistics in statistics_list:
            self.assertIn(statistics.station_code, station_codes)
            self.assertIsInstance(statistics.total_passengers, int)
            self.assertIsInstance(statistics.station_name, str)
    ...
    def test_database_connection_retry(self):
        ...
        with patch('taiwan_railway_gui.config.get_config') as mock_config:
            ...
            with patch('taiwan_railway_gui.dao.database_manager.psycopg2') as mock_psycopg2:
                mock_psycopg2.pool.ThreadedConnectionPool.side_effect = Exception("連線失敗")
